...
impl Pathtracer {
	pub fn new(width: u32, height: u32, frame_number: u32, number_of_threads_render: usize, number_of_threads_post_process: usize, continue_old_simulation: bool, post_process_data: Vec<(f32, i32)>, search_window_radius: i32, number_of_scales: i32, number_of_rays: u64, perform_post_process: bool, tracing: Tracing) -> Self {
		Self {
			width,
			height,
			frame_number,
	...
	fn render(&self, renderer_scene: &mut RendererScene) -> RendererOutput {
		let tm = now();
		...
		for i in 0_usize..self.number_of_threads_render {
			...
			let (width, height, number_of_rays, perform_post_process, write_percentage, tracing) = (self.width, self.height, self.number_of_rays, self.perform_post_process, i == 0, self.tracing);
			threads.push(spawn(move || {
				Renderer::new(width, height, number_of_rays, perform_post_process, write_percentage, tracing, renderer_scene_clone).render()
			}));
		}
	...
	fn write_to_image(&self, renderer_output: &mut RendererOutput, prefix: usize) {
		...
		make_file(self.width, self.height, renderer_output, &pixeldata_filename, &image_filename, self.number_of_rays*(self.number_of_threads_render as u64));
		let duration = now() - tm;
		println!("It took {}:{}:{}.{}.", duration.num_hours(), duration.num_minutes()%60, duration.num_seconds()%60, (duration.num_milliseconds()%1000)/10);
	}
	...
	fn post_process(&self, mut renderer_output: &mut RendererOutput, post_process_datum: (f32, i32)) {
		...
		//for max_distance_and_patch_radius in self.max_distances_and_patch_radii.iter() {
			let mut threads: Vec<JoinHandle<_>> = Vec::new();
			let rows_per_thread = (self.height/(self.number_of_threads_post_process as u32)) as i32;
